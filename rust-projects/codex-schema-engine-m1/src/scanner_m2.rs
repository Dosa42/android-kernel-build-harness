...
/// Runs Milestone 2 as a strict wizard.
///
/// Phase 1 discovers only Codex/.codex directories across Linux.
/// Phase 2 inventories protocol-relevant candidate files and patterns.
/// Phase 3 verifies Phase 1 + Phase 2 against the Codex protocol and installed runtime.
...
fn metadata_with_retry(
    ...
    for attempt in 0..IO_ATTEMPTS {
        ...
        match result {
            ...
            Err(error) => {
                let operation = if follow_symlink { "follow directory symlink" } else { "inspect directory candidate" };
